...
class musicInstance:
    ...
    def play_Song(self) -> None:
        if len(self.queue) > 0:
            ...
            asyncio.run_coroutine_threadsafe(self.channel.send(content=f"Now playing **{result[0]}**", view=self.View(instance=self)), self.bot.loop)
    ...
    def next_Song(self) -> None:
        ...
        if len(self.queue) > 0:
            ...
            try: #download song
                msg = asyncio.run_coroutine_threadsafe(self.channel.send("Moving on to the next."), self.bot.loop).result()
                result = youtubeDl.youtubeAPI(keyword=self.queue.pop(0), guild_id=self.guild_id)
                ...
                else: asyncio.run_coroutine_threadsafe(msg.edit(content="Song not found"))
                self.next_Song()
                return
